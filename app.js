...
let dishesWithChickpeas = problemNine();
console.log(dishesWithChickpeas)
...
function problemTen(){
    ...
    let allServing = result.reduce(function(total, sum){
        return total + sum;
    })
    return allServing
}

let totalServing = problemTen();
...
function problemEleven(){
    ...
    let currentCuisineTypes = []
    return dishes.filter(dish => {
        if(currentCuisineTypes.indexOf(dish.cuisine) == -1) {
            currentCuisineTypes.push(dish.cuisine);
            return true
        } else {
            return false
        }
...
let differentCuisines = problemEleven();
console.log(differentCuisines)